...
def _update_chart_filter(
    ...
    *,
    ...
    chart_selection_state = st.session_state.setdefault("chart_selection_state", {})
...
def _render_operating_assets_view() -> None:
    ...
    if capacity_col and capacity_col in filtered_assets.columns:
        ...
        if fuel_col and fuel_col in fuel_mw_df.columns:
            fuel_mw_plot = (
                fuel_mw_df.groupby(fuel_col, dropna=False)[capacity_col]
                .sum(min_count=1)
                .reset_index()
            ...
            fuel_mw_chart_kwargs: dict[str, Any] = {}
            if fuel_mw_color_col:
                fuel_mw_chart_kwargs = {
                    "color": fuel_mw_color_col,
                    "color_discrete_map": {"Selected": "#4C78A8", "Other": "#9AA4B2"},
                    "category_orders": {fuel_mw_color_col: ["Selected", "Other"]},
                }
            fuel_mw_fig = px.bar(
                fuel_mw_plot,
                x=capacity_col,
...
def _render_generation_fleet_view() -> None:
    ...
    st.caption("Generation fleet data comes from ERCOT MORA Resource Details.")
    st.markdown("[ERCOT MORA Resource Adequacy Page](https://www.ercot.com/gridinfo/resource)")

    with st.sidebar:
        st.header("Generation Fleet Source")
        operating_index_url = st.text_input(
            "Operating Fleet URL",
        ...
        if st.button("Refresh MORA Fleet", use_container_width=True):
            ...
            try:
                ...
                if previous_source_url and (
                    ...
                    st.session_state["generation_operating_refresh_notice"] = (
                        "No new MORA workbook found. Using current generation fleet data."
                    )
                else:
                    _cached_fetch_operating_assets.clear()
                    st.session_state["generation_operating_refresh_notice"] = "MORA generation fleet refresh complete."
            except Exception as exc:  # pylint: disable=broad-except
                st.session_state["generation_operating_refresh_notice"] = (
                    "Could not check for a newer MORA workbook. "
                    f"Using current data. Details: {exc}"
                )

    operational_notice = st.session_state.get("generation_operating_refresh_notice")
    if operational_notice:
        if operational_notice.endswith("refresh complete."):
    ...
    try:
        ...
        st.error(f"Could not load MORA generation fleet from ERCOT: {exc}")
        return

    if operational_df.empty:
        st.info("No MORA generation fleet rows were returned.")
        return
    ...
    if cod_year_col and cod_year_col in filtered_op.columns:
        ...
        if not years.empty:
            ...
            selected_year = st.slider(
                "COD/In-Service Year Range (MORA)",
                min_value=min_year,
                max_value=max_year,
                value=bounded,
                key=key,
    ...
    op_metric_cols[0].metric("MORA Capacity (MW)", f"{op_capacity:,.0f}" if op_capacity is not None else "n/a")
    op_metric_cols[1].metric("MORA Projects", f"{len(filtered_op):,}")
    ...
    st.caption(
        f"MORA rows shown: {len(filtered_op):,} / {len(operational_df):,} | "
    ...
    st.download_button(
        "Download MORA Fleet (CSV)",
        data=filtered_op.to_csv(index=False).encode("utf-8"),
        file_name="ercot_generation_fleet_mora.csv",
        mime="text/csv",
    )
...
if fuel_display_col and fuel_display_col in fuel_chart_df.columns:
    ...
    if _update_chart_filter("fuel", "fuel_chart", fuel_selected, event=fuel_event):
        st.rerun()
else:
...
if technology_display_col and technology_display_col in technology_chart_df.columns:
    ...
    if _update_chart_filter("technology", "technology_count_chart", technology_count_selected, event=technology_count_event):
        st.rerun()
else:
...
if technology_display_col and technology_display_col in technology_chart_df.columns:
    if capacity_col and capacity_col in technology_chart_df.columns:
        ...
        if _update_chart_filter("technology", "technology_mw_chart", technology_mw_selected, event=technology_mw_event):
            st.rerun()
    else:
...
if cod_col and cod_col in cod_chart_df.columns:
    ...
    if not cod_data.empty:
        ...
        if capacity_col and capacity_col in cod_data.columns:
            ...
            if _update_chart_filter("cod_year", "expected_cod_year_mw_chart", cod_mw_selected, event=cod_mw_event):
                st.rerun()
            ...
            if _update_chart_filter("cod_year", "expected_cod_year_projects_chart", cod_count_selected, event=cod_count_event):
                st.rerun()
        ...
        if _update_chart_filter("cod_year", "timeline_chart", timeline_selected, event=timeline_event):
            st.rerun()
...
if dev_col and dev_col in developer_chart_df.columns:
    ...
    if capacity_col and capacity_col in developer_chart_df.columns:
        ...
        if _update_chart_filter("developer", "developer_mw_chart", dev_mw_selected, event=dev_mw_event):
            st.rerun()
    ...
    if _update_chart_filter("developer", "developer_count_chart", dev_count_selected, event=dev_count_event):
        st.rerun()
else:
...
if zone_col and zone_col in zone_chart_df.columns:
    if capacity_col and capacity_col in zone_chart_df.columns:
        ...
        if _update_chart_filter("zone", "zone_chart", zone_selected, event=zone_event):
            st.rerun()
    else:
